...
async function layout(settings: LayoutSettings) {
	const paragraphs = settings.text
...
async function layoutParagraph(paragraph: string, settings: LayoutSettings) {
	...
	while (words.length > 0) {
		...
		if (nextWord === undefined) {
			throw new Error(
				"Infinite loop. Invalid array length. This state should not be reachable",
			);
		}

		const isStartOfLine = currentLine === empty;
		const spaceToInsert = isStartOfLine ? empty : space;
		const remainingLineSpace =
			settings.width - currentLine.length - spaceToInsert.length;
		...
		if (nextWord.length <= remainingLineSpace) {
			...
			if ((hyphenate || isStartOfLine) && remainingLineSpace > 0) {
				result = await hyphenateToFit(nextWord, remainingLineSpace);
...
async function hyphenateToFit(
	...
	for (
		...
		const lengthOfRemaining =
			stringArrayLength(remainingWord) + separator.length;
...
function alignJustify(text: string, settings: LayoutSettings): string {
	...
	const metaSplitText: Array<SplitTextWithMeta> = splitText.map((text) => ({
		text,
		addedSpaces: 0,
	}));
	...
	const textWithExtraSpaces = metaSplitText.map((textWithMeta, index) => {
		...
		return (
			space.repeat(spacesBefore) + textWithMeta.text + space.repeat(spacesAfter)
		);
	});
...
export function useMonoglyph(
	...
	watch(
		() => [toValue(text), toValue(width)] as [string, number],
		async ([textValue, widthValue]) => {
			if (textValue.trim() !== "" && widthValue >= 1) {
				const layoutSettings: LayoutSettings = {
					addSpacesOnRight: false,
					align: "justify",
					hyphenate: "adaptive",
					text: textValue,
					width: widthValue,
				...
				formattedText.value = await layout(layoutSettings);
			}
		},
		{ immediate: true },
	);

	return readonly(formattedText);
}
